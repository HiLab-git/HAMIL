...
class Backbone(nn.Module, ABC_Model):
    def __init__(self, model_name, num_classes=20, mode='fix', segmentation=False):
        ...
        if 'resnet' in model_name:
            ...
            #state_dict = torch.load('/mnt/data1/lanf_zhong/label_supervision_segmentation/WSSS_vgg/checkpoints2/train_cls/resnet50.pth', map_location='cpu')
...
class DeepLabv3_Plus(Backbone):
    def __init__(self, model_name, num_classes=4, mode='fix', use_group_norm=False):
...
class DeepLabv3_Plus_MultiHead(Backbone):
    def __init__(self, model_name, num_classes=4, mode='fix', use_group_norm=False):
        super().__init__(model_name, num_classes, mode, segmentation=False)
        ...
        self.aspp = ASPP(output_stride=16, norm_fn=norm_fn_for_extra_modules)
        self.decoder1 = Decoder(num_classes, 256, norm_fn_for_extra_modules)
        self.decoder2 = Decoder(num_classes, 256, norm_fn_for_extra_modules)
        self.decoder3 = Decoder(num_classes, 256, norm_fn_for_extra_modules)
...
class DeepLabv3_Plus_Pre(nn.Module):
    def __init__(self, num_classes=4):
        ...
        self.stage1 = nn.Sequential(model.model.conv1, 
                                    model.model.bn1, 
                                    model.model.relu, 
                                    model.model.maxpool)
        self.stage2 = nn.Sequential(model.model.layer1)
        self.stage3 = nn.Sequential(model.model.layer2)
        self.stage4 = nn.Sequential(model.model.layer3)
        self.stage5 = nn.Sequential(model.model.layer4)

        self.aspp = ASPP(output_stride=16, norm_fn=group_norm)
        self.decoder = Decoder(num_classes, 256, group_norm)
